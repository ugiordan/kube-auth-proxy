...
| flag: `--disable-redirect-on-csrf-error`<br/>toml: `disable_redirect_on_csrf_error` | bool           | on OAuth callback CSRF failure (e.g. missing or expired CSRF cookie), return 403 with the error page instead of redirecting to sign-in. Default is to redirect (302) so the user can retry; set this when you need the 403 error page with "Go back" (e.g. when not behind ext_authz).                                                                                                                                                                                                                    | false       |
